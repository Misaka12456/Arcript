...
public class TreeViewControl : MonoBehaviour
{
	...
	private void Start()
	{
		if (Template.activeSelf) Template.SetActive(false); // 隐藏可能在编辑模式下显示的模板
	}
	...
	public bool ItemIsCheck(string itemName)
	{
		...
		{
			if (_treeViewItems[i].transform.Find("TreeViewText").GetComponent<TMPText>().text == itemName)
			{
	...
	public List<string> ItemsIsCheck()
	{
		...
		for (int i = 0; i < _treeViewItems.Count; i++)
		{
			...
			{
				items.Add(_treeViewItems[i].transform.Find("TreeViewText").GetComponent<TMPText>().text);
			}
		}
	...
	public void GenerateTreeView()
	{
		...
		for (int i = 0; i < Data.Count; i++)
		{
			...
			item.transform.Find("TreeViewText").GetComponent<TMPText>().text = Data[i].Name;
			item.transform.SetParent(TreeItems);
			item.transform.localPosition = Vector3.zero;
			item.transform.localScale = Vector3.one;
			item.transform.localRotation = Quaternion.Euler(Vector3.zero);
			item.GetComponent<TreeViewItem>().Data = Data[i];
			item.SetActive(true);
	...
	//public void GenerateTreeViewByFolderStructure(string basePath, string rootName = "")
	//{
	//	Data = new List<TreeViewData>();
	//	GenerateTreeViewByFolderStructure(basePath, -1, rootName);
	//	GenerateTreeView();
	//	RefreshTreeView();
	//}

	public void GenerateTreeViewByFolderStructure(string basePath, string rootName = "Project Root")
	{
		var folderDataList = GetFolderData(basePath);
		var treeViewDataList = folderDataList.Select((fd, i) => new TreeViewData(this)
		{
			Name = fd.Name,
			Tag = fd.Path,
			ParentID = fd.ParentID + 1 // 由于Root的ParentID为-1，因此每个子项ID都加1。
		}).ToList();
		
		for (int i = 0; i < treeViewDataList.Count; i++)
		{
			treeViewDataList[i].ParentID -= 1;
		}
		treeViewDataList[0].Name = rootName;
		Data = treeViewDataList;
		GenerateTreeView();
		RefreshTreeView();
	}

	private List<FolderData> GetFolderData(string path)
	{
		var folderDataList = new List<FolderData>();
		var root = new DirectoryInfo(path);
		var queue = new Queue<(DirectoryInfo, int)>();

		folderDataList.Add(new FolderData { Path = root.FullName, Name = root.Name });

		queue.Enqueue((root, 0));

		while (queue.Count > 0)
		{
			var (currentDir, parentId) = queue.Dequeue();
			var subDirectories = currentDir.GetDirectories();

			foreach (var sub in subDirectories)
			{
				folderDataList.Add(new FolderData { Path = sub.FullName, Name = sub.Name, ParentID = parentId });
				queue.Enqueue((sub, folderDataList.Count - 1));
			}
		}
